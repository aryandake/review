...
namespace Fiction2Fact.Projects.Certification
{
    ...
    public partial class Certification_CertificationCUApproval : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (Request.QueryString["Type"] != null)
                {
                    ...
                    {
                        this.lblMsg.Text = "No records pending for your review.";
        ...
        private void getCertification()
        {
            ...
            dtCertDashboard = certBL.getCertificationForPendingApproval("L4", Page.User.Identity.Name.ToString(),
                            strConnectionString);
        ...
        protected void gvCertDashboard_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            utilVO.setCode(" and CDM_ID = " + hfDepartmentID.Value);
        ...
        private void bindApprovalGrid()
        {
            try
            {
                ...
                dt = certBL.getCertificationsApproval(hfDepartmentID.Value, "L4", Page.User.Identity.Name.ToString(),
                    strConnectionString);
                ...
                for (int intCnt = 0; intCnt < intExcCnt; intCnt++)
                {
                    ...
                    strHtmlTableRows = strHtmlTableRows + "<tr>" +
                        ...
                        "<td class='tabbody3'><label ID='lblDeptName" + uniqueRowId + "' class = 'textbox1' >" + strUHDept + "</label></td>" +
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
        {
            ...
            //bindApprovalGrid();
            getCertification();
            writeError("Certificate(s) reviewed successfully.");
        }
        ...
        private void updateCertificationData(string strOperationType)
        {
            try
            {
                string strIds = "", strDeptName = "", strDeptId = "", strQuarter = "", strFunctionName = "";
                ...
                for (int intIndex = 0; intIndex < gvCertApproval.Rows.Count; intIndex++)
                {
                    ...
                    if (RowLevelCheckBox.Checked)
                    {
                        ...
                        HiddenField hfFunctionId = (HiddenField)(gvr.FindControl("hfFunctionId"));
                        ...
                        if (strOperationType.Equals("R"))
                        {
                            ...
                            SendRejectionMail(lblId.Text, strDeptId, strDeptName, strQuarter, strRetVal);
                        }
                ...
                if (strOperationType.Equals("A"))
                {
                    if (!strIds.Equals(""))
                    {
                        ...
                        if (strRetVal.Equals("L4A"))
                        {
                            SendIntimationMailToEC(strIds, hfFunctionId.Value, strFunctionName, strQuarter);
                        }
                    }
                }
            }
        ...
        private bool CheckIfAllCertificatesApproved(string strSubDeptId)
        {
            ...
            if (strDeptId != "" && strDeptId != null)
            {
                ...
                strApprovedCnt = DataServer.ExecuteScalar(" select Count(1) from [TBL_CERTIFICATIONS] " +
                                                          ...
                                                          " inner join TBL_CERT_MAS on CERTM_ID = CERT_CERTM_ID and CERTM_LEVEL_ID=1 " +
                                                          " inner join TBL_CERT_SUB_DEPT_MAS on CERTM_DEPT_ID = CSDM_ID " +
                                                          " inner join TBL_CERT_DEPT_MAS on CDM_ID = CSDM_CDM_ID and CERT_STATUS = 'L3A' and CDM_ID = "
                                                          + strDeptId).ToString();
        ...
        private void SendRejectionMail(string strCertIds, string strDepartmentId, string strDepartment, string strQuarter, string strRejectionLevel)
        {
            try
            {
                ...
                {
                    cc.ParamMap.Add("ConfigId", 1094);
                    cc.ParamMap.Add("To", "Level3");
                    cc.ParamMap.Add("cc", "Comp,Level2,Level1");
                }
        ...
        protected void gvCertApproval_DataBound(object sender, EventArgs e)
        {
            ...
            for (int intIndex = 0; intIndex < gvCertApproval.Rows.Count; intIndex++)
            {
                ...
                if (hfStatus.Value.Equals("L3A"))  //Submitted
                {
                ...
                if (!hfStatus.Value.Equals("L3A")) //Submitted & Approved By HOD
                {
                    IsApprovalBtnShow = "False";
                }

                if (hfStatus.Value.Equals("L5R"))
                {
                    IsApprovalBtnShow = "True";
                    IsApprovalRejectBtnShow = "True";
                    RowLevelCheckBox.Enabled = true;
                    lnlEdit.Visible = true;
                }
        ...
        private void SendIntimationMailToEC(string strCertIds, string strDepartmentId, string strDepartment, string strQuarter)
        {
            try
            {
                CommonCode cc = new CommonCode();
                Authentication auth = new Authentication();
                //<<Modified by Ankur Tyagi on 01Feb2024 for CR_1945
                cc.ParamMap.Add("ConfigId", 1090);
                cc.ParamMap.Add("To", "EC");
                cc.ParamMap.Add("cc", "Comp,Level3,Level2,Level1");
                cc.ParamMap.Add("SubmittedBy",
                               (new Authentication()).getUserFullName(Page.User.Identity.Name));
