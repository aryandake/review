...
{
    [WebService(Namespace = "Fiction2Fact.Legacy_App_Code.Circulars")]
    [WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]

    [System.Web.Script.Services.ScriptService]
    public class AJAXDropdownCirculars : System.Web.Services.WebService
    {
        private string mstrConnectionString = null;
        UtilitiesBLL UtilitiesBL = new UtilitiesBLL();
        public AJAXDropdownCirculars()
        {
        ...
        [WebMethod]
        public List<string> getCircularDetailsFromSubjectAndCircularNo(string prefixText)
        {
            ...
            {
                DB.F2FCommand.CommandText = " SELECT CM_TOPIC + ' (' + CM_CIRCULAR_NO + ')' as [Name], CM_ID as [CircularName] FROM TBL_CIRCULAR_MASTER WHERE CM_STATUS = 'A' " +
                                            " AND (CM_TOPIC LIKE '%' + @SearchText + '%' OR CM_CIRCULAR_NO LIKE '%' + @SearchText + '%') ";
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string item = AutoCompleteExtender.CreateAutoCompleteItem(dr["Name"].ToString(), dr["CircularName"].ToString());
                    list.Add(item);
                }
            }
        ...
        public string[] GetIssuingAuthority(string prefixText)
        {
            ...
            strSQL = "Select CIA_NAME from TBL_CIRCULAR_ISSUING_AUTHORITIES where CIA_STATUS='A' AND CIA_NAME like '" + prefixText + "%'";
            ...
            (string knownCategoryValues, string category)
        {
            ...
            try
            {
                ...
                if (strLevel.Equals("0"))
                {
                    ...
                    " OR CDM_CXO_USERID = '" + strLoggedInUser + "')" +
                    " order by CSSDM_NAME";
                }
                else if (strLevel.Equals("1"))
                {
